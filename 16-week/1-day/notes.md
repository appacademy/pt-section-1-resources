...
In addition to using .then chains, we can use promises by defining our function with the `async` keyword:

```js
const add = async () => {};
```

This turns our function into an asynchronous function, this makes our function return a promise object.

Within the async function, we can achieve the same behavior as a .then chain using the `await` keyword:
