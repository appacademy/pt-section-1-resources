...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (req.method === 'GET' && req.url === '/dogs') {
      // 1. Getting the dogs.html
      const template = fs.readFileSync("./views/dogs.html", "utf-8")
      // 2. Replace #{dogList} with li's with the dog names
      const dogList = dogs.map(dog => {
        return `<li>${dog.name}</li>`
      })
      const resBody = template.replace(/#{dogsList}/g, dogList.join(""))

      // 3. Send the updated html as a response
      res.statusCode = 200;
      res.setHeader("Content-Type", "text/html");
    ...
    if (req.method === 'GET' && req.url === '/dogs/new') {
      const form = fs.readFileSync("./views/create-dog.html", "utf-8");
      res.statusCode = 200;
      res.setHeader("Content-Type", "text/html");
      return res.end(form);
    }
    ...
    if (req.method === 'GET' && req.url.startsWith('/dogs/')) {
      ...
      if (urlParts.length === 3) {
        ...
        const resBody = template.replace(/#{name}/g, dog.name).replace(/#{age}/g, dog.age);
        res.statusCode = 200;
        res.setHeader("Content-Type", "text/html");
        return res.end(resBody);
      }
